...
def add_new_funsraising(data):
    try:
        ...
        fundraising.account = make_new_fundraising_account(data.title)
        fundraising.goal = data.goal
        fundraising.thumbnail = data.thumbnail

        if fundraising.account is None:
...
def make_new_fundraising_account(account_name):
    try:
        user = get_user_info()

        parent_account = "1180.000 - Penggalangan Dana - Madina"
        ...
        if child_accounts:
            ...
            next_account_number = f"1180.{last_account_number + 1:03}"
        else:
            next_account_number = "1180.001"
